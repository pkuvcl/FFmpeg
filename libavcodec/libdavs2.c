...
static av_cold ff_davs2_init(AVCodecContext *avctx)
{
    ...
    av_log(avctx, AV_LOG_VERBOSE, "decoder created. 0x%p\n", cad->decoder);
    return 0;
}

static int DumpFrames(AVCodecContext *avctx, davs2_picture_t *pic, davs2_seq_info_t *headerset, AVFrame *frame)
{
    DAVS2Context *cad = avctx->priv_data;
    avctx->flags |= AV_CODEC_FLAG_TRUNCATED;
...
static av_cold ff_davs2_end(AVCodecContext *avctx)
{
    DAVS2Context *cad = avctx->priv_data;
    ...
    if (cad->decoder) {
        ...
        av_log(avctx, AV_LOG_VERBOSE, "decoder destroyed. 0x%p; frames %d\n", cad->decoder, cad->decoded_frames);
        cad->decoder = NULL;
    }
    ...
    if (!buf_size) {
        ...
        for (;;) {
            ...
            if (cad->out_frame.ret_type != DAVS2_DEFAULT) {
                *got_frame = DumpFrames(avctx, &cad->out_frame, &cad->headerset, frame);
